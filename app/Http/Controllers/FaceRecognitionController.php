...
class FaceRecognitionController extends Controller
{
    ...
    public function trainAdmin()
    {
        ...
        $output = \App\Services\PythonRunner::runRaw($scriptPath);
    ...
    public function processScanner(Request $request)
    {
        ...
        if (isset($output['success']) && $output['success'] &&
            ...
            $user = \App\Models\User::with(['siswa', 'guru'])->find($output['user_id']);
            if (!$user) {
                @unlink($tempPath);
    ...
    public function verifySiswaAuth(Request $request)
    {
        $request->validate([
            'image' => 'required|string',
        ]);
    ...
    public function registerFaceDataset(Request $request)
    {
        $request->validate([
            'image' => 'required|string',
        ...
        if ($sample >= 20) {
            ...
            if (file_exists($trainScript)) {
                \App\Services\PythonRunner::runRaw($trainScript);
                $isTrained = true;
            }
        }
