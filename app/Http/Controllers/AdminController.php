...
class AdminController extends Controller
{
    public function downloadLaporanBulananHadir()
    {
        ...
        foreach ($absensis as $absen) {
            ...
            if ($absen->status == 'hadir') {
                $rekapSiswa[$siswaId]['hadir']++;
            } elseif ($absen->status == 'terlambat') {
                $rekapSiswa[$siswaId]['terlambat']++;
            } elseif ($absen->status == 'alpha') {
                $rekapSiswa[$siswaId]['alpha']++;
            }
        }

        $pdf = Pdf::loadView('admin.laporan_pdf', compact('rekapSiswa', 'namaBulan'));
    ...
    public function storeGuru(StoreGuruRequest $request)
    {
        ...
        if ($request->has('face_samples') && count($request->face_samples) > 0) {
            ...
            if (file_exists($pythonScriptPath)) {
                $output = PythonRunner::runRaw($pythonScriptPath);
    ...
    public function destroyGuru($id)
    {
        ...
        $pythonScriptPath = storage_path('app/public/train.py');
        if (file_exists($pythonScriptPath)) {
            PythonRunner::runRaw($pythonScriptPath);
        }

        return back()->with('success', 'Data Guru dan biometrik wajah berhasil dihapus.');
    }

    public function bulkDestroyGuru(BulkDestroyGuruRequest $request)
    {
    ...
    public function bulkDestroySiswa(BulkDestroySiswaRequest $request)
    {
